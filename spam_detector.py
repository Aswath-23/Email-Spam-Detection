...
def main():
    ...
    try:
        ...
        try:
            ...
            if missing_cols:
                print(f"Error: The following required columns are missing from your CSV: {', '.join(missing_cols)}")
                print(f"Columns found: {', '.join(df.columns)}")
                print("Please ensure your CSV has columns: subject, body, label")
                return
        except FileNotFoundError:
    ...
    # If label column is numeric (1/0), convert to spam/ham
    if df['label'].dtype in ['int64', 'float64'] or df['label'].apply(lambda x: str(x).isdigit()).all():
